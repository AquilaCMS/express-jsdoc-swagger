...
const globFilesMatches = async (
    baseDir,
    filePaths,
    excludedFolder = DEFAULT_EXCLUDED_FOLDER,
) => {
    ...
    if (filePaths.length > 0) {
        ...
        while (i + 1 <= filePaths.length) {
            ...
            const files = glob.sync(path.resolve(baseDir, filePath), {
                ignore: '**/node_modules/**',
            });
            const filterFiles = files.filter(
                (file) => !file.includes(excludedFolder),
            );
