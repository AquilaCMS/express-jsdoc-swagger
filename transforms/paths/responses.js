...
const hasOldContent = (value, status) =>
    value[status] && value[status].content;

const EXAMPLE_DIVIDER = ' | ';
const mapExample = (example) =>
    (example || '').split(EXAMPLE_DIVIDER);
...
const formatResponses = (values) => {
    ...
    while (i < values.length) {
        ...
        const [descriptions, exampleName, exampleValue] = mapExample(
            value.description,
        );
        const [status, description, contentType] = mapDescription(
            descriptions,
        );
        if (!STATUS_CODES[status]) {
            errorMessage(
                `Status ${status} is not valid to create a response`,
            );
            return {};
        }
        const content = {
        ...
        if (exampleName && exampleValue) {
            content[status].content[
                Object.keys(content[status].content)[0]
            ].examples = {
                [exampleName]: {
                    value: exampleValue,
                },
