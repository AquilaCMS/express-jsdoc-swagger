...
const wrongInOption = (paramValue) => {
    if (!paramValue.includes('request.body')) {
        errorMessage(
            `If you want to add one @param as body you must provide "request.body" instead of ${paramValue}`,
        );
    }
...
const parseParameter = (param) => {
    ...
    const [description, enumValues, defaultValue] = formatDescription(
        param.description,
    );
    const options = {
        name,
    ...
    const schema = getSchema('@param', param.name)(
        param.type,
        enumValues,
        defaultValue,
    );
