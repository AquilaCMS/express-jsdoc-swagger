...
const addRefSchema = (typeName, applications, elements) => {
    if (!typeName && !elements)
        return { items: formatRefSchema(applications) };
    return {};
};
...
const formatProperties = (properties) => {
    ...
    while (i < properties.length) {
        ...
        const [description, format] = mapDescription(
            descriptionValue,
        );
        response = {
            ...response,
...
const formatRequiredProperties = (requiredProperties) =>
    requiredProperties.map(getPropertyName);
    ...
    return {
        [typedef.name]: {
            ...
            ...(requiredProperties.length
                ? {
                      required: formatRequiredProperties(
                          requiredProperties,
                      ),
                  }
                : {}),
