...
const formatDescription = (description) => {
    ...
    const descriptionValue = descriptionTypes
        .filter(
            (value) =>
                !value.includes(ENUM_IDENTIFIER) &&
                !value.includes(DEFAULT_IDENTIFIER),
        )
        .join(DESCRIPTION_DIVIDER);
    const enumOption = descriptionTypes.find((value) =>
        value.includes(ENUM_IDENTIFIER),
    );
    let enumValues;
    if (enumOption) {
    ...
    const defaultOption = descriptionTypes.find((value) =>
        value.includes(DEFAULT_IDENTIFIER),
    );
    let defaultValue;
    if (defaultOption) {
