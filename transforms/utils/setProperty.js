...
const setProperty = (entity) => {
    const requiredError = (key, item) =>
        new Error(
            `Key ${key} is required in item ${JSON.stringify(
                item,
            )} for Entity ${entity}`,
        );
    ...
    const warnType = (type, value) =>
        errorMessage(
            `${type} is not valid with value ${value} for Entity ${entity}`,
        );

    return (item, key, options) => {
        if (!item || !key || !options) {
            const requiredParamsError = new Error(
                'item, key and options para are required',
            );
            throw requiredParamsError;
        }
        const value = item[key];
        if (options.required && value === undefined)
            throw requiredError(key, item);
