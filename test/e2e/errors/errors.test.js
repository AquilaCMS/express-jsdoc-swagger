...
test('should give a nice error message for responses', async () => {
    ...
    expect(console.warn).toHaveBeenNthCalledWith(
        1,
        chalk.yellow(
            '[express-jsdoc-swagger] Entity: @return could not be parsed. Value: "200 - success response - application/json" is wrong',
        ),
    );
    expect(console.warn).toHaveBeenNthCalledWith(
        2,
        chalk.yellow(
            '[express-jsdoc-swagger] Entity: @return could not be parsed. Value: "400 - Bad request response" is wrong',
        ),
    );
});
...
test('should give a nice error message for parameters', async () => {
    ...
    expect(console.warn).toHaveBeenNthCalledWith(
        1,
        chalk.yellow(
            '[express-jsdoc-swagger] Entity: @param could not be parsed. Value: "name.query.required" is wrong',
        ),
    );
    expect(console.warn).toHaveBeenNthCalledWith(
        2,
        chalk.yellow(
            '[express-jsdoc-swagger] Entity: @param could not be parsed. Value: "phone.param" is wrong',
        ),
    );
});
